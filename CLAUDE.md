...
## 프로젝트 목표
익숙하지 않은 영역(JSON 파싱/저장)을 직접 구현해보며 이해한 뒤, 그 구조를 그대로 유지한 채
JSON 파일을 데이터 저장소로 사용하는 CRUD 콘솔 애플리케이션으로 확장한다.

진행 순서는 반드시 아래 2단계를 지킨다.

1. **PoC 단계**: JSON 파싱 / 파일 저장 기능을 외부 라이브러리 없이 직접 구현하고 동작을 이해한다.
...
- JSON 처리: 외부 라이브러리를 쓰지 않고 직접 구현한다 (파싱, 직렬화 모두 자체 구현).
  이 과제의 핵심 학습 목표가 "라이브러리 없이 JSON 처리 로직을 이해하는 것"이므로
  nlohmann/json 등 기존 JSON 라이브러리는 사용하지 않는다.
...
- PoC 단계 산출물(JsonValue/JsonParser/JsonWriter/JsonStore/Record)은
  루트에 그대로 유지한다.
- CRUD 기능(Create/Read/Update/Delete)부터는 기능별 디렉토리로
  분리한다. 테스트도 같은 이름의 하위 폴더로 미러링한다.
- 구조:
  ```
  AIProject5/
    AIProject5.cpp        # main, 메뉴 루프
    JsonValue.h / .cpp      # JSON 값 표현 (object/array/string/number/bool/null)
    JsonParser.h / .cpp     # 문자열 -> JsonValue 파싱
    JsonWriter.h / .cpp     # JsonValue -> 문자열 직렬화
    JsonStore.h / .cpp     # JSON 파일 read/write, PoC에서 만든 로직 그대로 재사용
    Record.h / .cpp        # 데이터 모델 + readAllRecords/writeAllRecords
                             # (Create/Read/Update/Delete 공통 저장소 접근점)
    Create/                  # Create 기능
      Create.h / .cpp
    Read/                    # Read 기능
      Read.h / .cpp
    ...
    tests/                   # GoogleTest 테스트 코드
      Create/, Read/, Update/, Delete/   # 기능별 테스트, 소스 구조와 미러링
      Integration/             # CRUD 기능을 이어서 검증하는 통합 테스트
...
## 데이터 모델
- `Record { int id; std::string name; std::string phone; }` — 연락처를 표현.
- `id`는 프로그램이 자동 생성한다 (기존 레코드 최댓값 + 1, 개수 기준이 아님). 사용자 입력을 받지 않는다.
- 필드 추가가 필요해지면 `Record`, `toJson`/`fromJson`, 관련 스펙 문서를 함께 갱신한다.
...
- `main()`(`AIProject5.cpp`)은 인자 없이 실행하면 콘솔 메뉴, 인자가 하나라도 있으면 GoogleTest를
  실행한다. 특정 플래그 이름(`--test` 등)이 아니라 "인자 존재 여부"로 분기해야 Visual Studio
  Test Explorer의 GoogleTest 어댑터(`--gtest_*` 플래그로 호출)와도 정상 동작한다.
- 주석은 WHY가 필요한 경우에만 한국어로 작성한다. 코드가 스스로 설명하는 내용은 주석으로
  반복하지 않는다. 한글 주석 인코딩 경고를 막기 위해 프로젝트 전체에 `/utf-8` 컴파일 옵션이
  적용되어 있다 (vcxproj의 모든 구성에 `AdditionalOptions`로 설정됨).
...
- nlohmann/json 등 외부 JSON 라이브러리 사용 (직접 구현이 이 과제의 핵심 학습 목표).
