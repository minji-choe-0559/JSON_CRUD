...
## 테스트 케이스 목록
- [x] 정상 케이스: 각 타입(null/bool/number/string/array/object)으로
      생성 시 해당하는 `is*()`가 true 반환
- [x] 정상 케이스: 각 타입 생성 후 `as*()`로 원래 값 그대로 조회
- [x] 정상 케이스: object에서 존재하는 key로 값 조회
- [x] 정상 케이스: array에서 유효한 index로 값 조회
- [x] 엣지 케이스: 빈 array, 빈 object 생성 및 조회
- [x] 예외 케이스: 타입이 다른 `as*()` 호출 시 예외 발생
- [x] 예외 케이스: object에 없는 key 접근 시 예외 발생
- [x] 예외 케이스: array에 범위 밖 index 접근 시 예외 발생
